import { pgTable, integer, serial, timestamp, jsonb, text, pgEnum, uniqueIndex, index } from 'drizzle-orm/pg-core';
...
(t) => [
    ...
    index('events_city_state_start_idx').on(t.cityId, t.state, t.startAt).where(sql`${t.state} = 'scheduled'`),
    index('events_city_category_idx').on(t.cityId, t.category).where(sql`${t.state} = 'scheduled'`)
]);