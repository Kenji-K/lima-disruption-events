...
type DateRange = { start: PlainDate; end?: PlainDate; raw: string };
...
const toStartIso = ({ y, m, d }: PlainDate): string =>
    `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}T00:00:00-05:00`;
const toEndIso = ({ y, m, d }: PlainDate): string =>
    `${y}-${String(m).padStart(2, '0')}-${String(d).padStart(2, '0')}T23:59:00-05:00`;

/** Rule-based Spanish date extraction (no ML per the v1 fence). Tries range
 *  patterns first, then "a partir del", then the first bare "N de MONTH".
 *  Years default to the post's year; a date landing >60 days BEFORE the post
 *  rolls forward one year (December post about January closures). Exported for
 *  fixture tests. */
