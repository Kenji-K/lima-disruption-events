...
5. **Ship the wrap-up as one commit.** Stage this PLAN.md update plus any ARCHITECTURE.md or ADR additions made during the session and commit them together as a single `docs: record [unit] wrap-up + [non-obvious decisions]` commit (e.g. `docs: record Commit C wrap-up + Drizzle runtime client conventions`). The next session's pickup checklist (top of this file, step 2) reads `Last sync point` against HEAD — for that check to be meaningful, the wrap-up itself has to be the latest commit. The agent does not auto-fire this; the user pulls the trigger.
