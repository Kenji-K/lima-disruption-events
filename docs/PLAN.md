...
**Two prod facts still pending when this was written:**

1. **First prod run of the six new event sources** — they're deployed + registered; the next 06:00 cron tick populates them. To warm up earlier, run the README's manual-ingest one-liner (`fly ssh console …`) — this agent session was not permitted to exec into the prod host.
2. **Tier-2 acceptance verifier verdict (fresh-context subagent, 2026-06-11):** 5/6 PASS, criterion 1 PARTIAL **on timing only** — the six new event sources' first prod tick is the next 06:00 cron; "nothing left to build, only to observe." The verifier independently re-ran the dedup suppression tests (18/18) and confirmed the road-alert cron fired on its 2-hourly schedule in prod (14:15 Lima tick, on cadence). Its PLAN errata are fixed in this wrap-up. Follow-up it queued: after the next 06:00 tick, confirm `/sources` shows the 6 new sources fresh and joinnus stays `[]` on public `/events`; note `/sources` can't distinguish "registered, never ran" from "doesn't exist" (minor rollout blind spot).

**Next session = per the 2026-06-11 workshop demo-cut plan:** (1) web demo polish ≤1 day (backlog #4 below) → (2) Tier 3 thin slice if the gate holds (Tier-2 verifier-passed by EOD 2026-06-17; time-box 1.5d; auto-cut) → (3) docs day → (4) Loom. shadcn/ui applies from the next new web component.
...
3. **Region resolution hard-codes Lima** in `upsertEvents`; needs ADR-first backfill plan when the first non-Lima source lands. (Note: SUTRAN _road alerts_ are national but live outside `events`; the gob.pe news gates keep national institutions Lima-only, so the assumption still holds for events.)
