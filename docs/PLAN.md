...
1. ~~Observe the first post-ADR-011 06:00 tick~~ — OBSERVED 2026-06-12 ~10:45: all 12 sources fresh in a 25s tick, zero failures; `ingest_quarantine` gained exactly (mml, 79872, past-event) and `events` stayed clean — the gates rejected the INDULTOS false positive in prod on their first scheduled run. (`sutran-alerts` had one transient 2-hourly failure at 10:15 Lima; snapshot retained by design.)
