...
**Two-layer retry, accumulator-based.** Per-page: 1 initial attempt + N retries with exponential backoff (defaults `[250, 500, 1000]ms`). Then after every page in the window has been attempted, a single end-of-run pass over the accumulator (`fetchWithRetry(url, log, [])`). The asymmetry is the point: phase 1 retries cluster within ~2s and recover from sub-second blips; phase 2 fires after the rest of the run has elapsed and acts as a "wait it out" pass for upstream wobbles that lasted minutes. Anything still failing after phase 2 is logged warn and dropped; the run completes with whatever was successfully fetched. The shared implementation lives in `apps/api/src/ingest/fetch.ts` as `fetchWithRetry` (extracted when scraper #2 landed, 2026-06-10); every scraper consumes it.

**Per-source isolation in the runner.** `runIngestOnce` iterates a `SCRAPERS` list and runs scrape → Zod-validate → upsert *per source*, catching per-source failures and continuing — one source throwing (or producing schema-invalid output) never blocks the others' ingest. Extends the existing "a failed scrape must not block the next scheduled run" rule sideways across sources. Failed sources are reported in the run-complete log line (`failedSources`).

**Static venue maps for fixed-venue sources.** When a source's events happen at a known fixed set of venues, the scraper carries a static venue map (stadium/venue name, expected location-string for a defensive cross-check, OSM-verified `{lng, lat}`) rather than geocoding at scrape time — first instance `futbolperuano-venues.ts`; GTN pins its single venue point the same way. The cross-check (JSON-LD location string must contain the mapped substring) turns silent venue swaps into loud programmer errors. Coordinates come from OpenStreetMap (Nominatim lookups, document the verification date); region resolution stays in the upsert layer per ADR-005 (scrapers never write `regions`).
...
### API event contract lives in packages/shared

`apiEventSchema`/`ApiEvent` (in `packages/shared/src/api-event.ts`) is the canonical event shape the HTTP API serves: ISO-UTC date strings, PostGIS point unpacked to `{lng, lat} | null`. It is deliberately a *shared* schema because it is the contract between two consumers: `apps/api` re-exports it as the route response schema (so serialization and the OpenAPI spec are anchored to it) and `apps/web` parses every API response against it at the fetch boundary (so contract drift fails loudly in the client instead of rendering garbage). Don't fork the shape on either side — change the shared schema and let both ends fail the same way. `scrapedEventSchema` (scraper output) and `apiEventSchema` (API output) are different boundaries and stay separate schemas.

### Cron rides the Fastify lifecycle

Per the V1-BRIEF Tier-0 decision (one process, one logger): the daily ingest schedule is defined once in `apps/api/src/ingest/schedule.ts` (`createIngestTask`) and attached in `src/main.ts` — the API entrypoint — with an `onClose` hook that stops the task and drains the DB pool, so `app.close()` is the single shutdown path. `src/cron.ts` remains as a thin standalone dev runner over the same module (`pnpm -F api cron`) for running the scheduler without the HTTP server; it is not the production path. Revisit only if cron and API need to scale independently.
